...
RoBERTa-large : an encoder-only model trained with a Masked Language Modeling objective, an improved variant of BERT trained on more data for longer<br>
LLaMA 3 8B : a decoder-only large language model pretrained on roughly 15 trillion tokens

## Methods Compared

1-Full Fine-Tuning : updating all model parameters on the downstream task, used as the baseline<br>
2-LoRA (Low-Rank Adaptation) : injects small trainable low-rank matrices instead of updating all weights<br>
3-P-Tuning : a soft-prompting approach that learns continuous prompt embeddings rather than modifying the model's core weights
